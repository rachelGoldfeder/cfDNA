...
#usage Rscript merge3_methcpg_plot.R dir1 samp1 dir2 samp2 dir3 samp3 savedir
...
a1=as.data.frame(fread(paste0("bis_",samp1,".cpg.txt")))
b1=as.data.frame(fread(paste0("oxbis_",samp1,".cpg.txt")))
c1=as.data.frame(fread(paste0("hmc_sig_",samp1,".cpg.txt")))

setwd(paste0(dir2))

a2=as.data.frame(fread(paste0("bis_",samp2,".cpg.txt")))
b2=as.data.frame(fread(paste0("oxbis_",samp2,".cpg.txt")))
c2=as.data.frame(fread(paste0("hmc_sig_",samp2,".cpg.txt")))

setwd(paste0(dir3))

a3=as.data.frame(fread(paste0("bis_",samp3,".cpg.txt")))
b3=as.data.frame(fread(paste0("oxbis_",samp3,".cpg.txt")))
c3=as.data.frame(fread(paste0("hmc_sig_",samp3,".cpg.txt")))
...
hist1=hist(b1$V4,breaks=20,xlim=c(0,100))
hist1$density = hist1$counts/sum(hist1$counts)*100
hist2=hist(b2$V4,breaks=20,xlim=c(0,100))
hist2$density = hist2$counts/sum(hist2$counts)*100
hist3=hist(b3$V4,breaks=20,xlim=c(0,100))
hist3$density = hist3$counts/sum(hist3$counts)*100

max=max(max(hist1$density),max(hist2$density),max(hist3$density))

pdf(paste0("oxbis_",samp1,samp2,samp3,"_cpg.histall.pdf"))
par(mar=c(8.1, 4.1, 4.1, 2.1), xpd=TRUE)
...
pdf(paste0("hmc_sig_",samp1,samp2,samp3,"_cpg.hist.pdf"))
par(mar=c(8.1, 4.1, 4.1, 2.1), xpd=TRUE)
