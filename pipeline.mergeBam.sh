...
oxbis_fq1=$2
outPrefix=$3
email=${4:-'Rachel.Goldfeder@jax.org'} # Note, I removed the email doing anything - it now will just send the email to the submitter of the job. I'm leaving this here to catch the argument in case it is supplied
...
samtools sort -@ 11 $bis_fq1 > ${outPrefix}.bis.sort.bam
...
samtools sort -@ 11 $oxbis_fq1 > ${outPrefix}.oxbis.sort.bam
...
SECOND_bis=`qsub process.bis.sh`
SECOND_oxbis=`qsub process.oxbis.sh`
