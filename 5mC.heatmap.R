...
a=as.data.frame(fread(paste0(maindir,"oxbis_",samp1,".binning.txt")))
b=as.data.frame(fread(paste0(maindir,"oxbis_",samp2,".binning.txt")))
c=as.data.frame(fread(paste0(maindir,"oxbis_",samp3,".binning.txt")))

colnames(a)=c("chr","start","end","meth","unmeth","all","samp1")
colnames(b)=c("chr","start","end","meth","unmeth","all","samp2")
colnames(c)=c("chr","start","end","meth","unmeth","all","samp3")
...
write.table(as.data.frame(all),paste0("mcall_",samp1,samp2,samp3,".txt"),col.names = F,row.names=F, quote=F, sep = "\t")
...
png(paste0("mcall_",samp1,samp2,samp3,"_heatmap.png"),1000,1000)
superheat(my_all_r,pretty.order.rows = TRUE,pretty.order.cols=TRUE)
dev.off()

png(paste0("mcall_",samp1,samp2,samp3,"_heatmap_order.png"),1000,1000)
