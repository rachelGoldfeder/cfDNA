...
a=as.data.frame(fread(paste0(maindir,"mcall_",samp1,samp2,samp3,".gencode.bed")))
colnames(a)=c("chr","start","end","samp1","samp2","samp3","gchr","gstart","gend","gene","glength")

samp1genes=subset(a,samp1!=0&samp2==0&samp3==0)
write.table(as.data.frame(samp1genes),paste0("mconly_",samp1,".gencode.txt"),col.names = F,row.names=F, quote=F, sep = "\t")
...
write.table(as.data.frame(samp2genes),paste0("mconly_",samp2,".gencode.txt"),col.names = F,row.names=F, quote=F, sep = "\t")
...
write.table(as.data.frame(samp3genes),paste0("mconly_",samp3,".gencode.txt"),col.names = F,row.names=F, quote=F, sep = "\t")
...
write.table(as.data.frame(allgenes),paste0("mc_genes_",samp1,samp2,samp3,".txt"),col.names = F,row.names=F, quote=F, sep = "\t")

samp2only=subset(allgenes,variable.x=="0"&variable.y=="samp2"&variable=="0") 
samp2only=samp2only$value
write.table(as.data.frame(samp2only),paste0("mc_genes_",samp2,".txt"),col.names = F,row.names=F, quote=F, sep = "\t")

samp3only=subset(allgenes,variable.x=="0"&variable.y=="0"&variable=="samp3")
samp3only=samp3only$value
write.table(as.data.frame(samp3only),paste0("mc_genes_",samp3,".txt"),col.names = F,row.names=F, quote=F, sep = "\t")
