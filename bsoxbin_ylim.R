...
histPercent(x$val,col=rgb(0.128,0.128,0.128,1/4),main = paste0("Histogram of % 5mC & 5hmC CpG methylation: ",sampname),xlab="% mc & hmC per base")
dev.off()
...
for (i in 1:10) {
        percbs=nrow(subset(x,all>=i))/nrow(x)
        percox=nrow(subset(y,all>=i))/nrow(y)
        perc.mat[i,1]= i
        perc.mat[i,2]=percbs
        perc.mat[i,3]=percox
}

colnames(perc.mat)=c("cov","BS bin percent","oxBS bin percent")
write.table(perc.mat,paste0(sampname,"_bin_percent.txt"),row.names=FALSE, quote=FALSE,sep="\t")
